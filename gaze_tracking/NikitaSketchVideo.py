...
def main():
    ...
    video_path = "2025-05-27 13-38-47.flv"

    time_name = datetime.datetime.now().strftime("%I_%M%p_%B_%d_%Y")
    tracker = EnhancedGazeTracker()
    coefs = np.load("D:\Code_University\IOT_project\Timo_Simo\calib_data.npz.npy")
    tracker.x_coeff = coefs[0]
    tracker.y_coeff = coefs[1]
    tracker.calibrating = False
    tracker.calibration_complete = True
